...
VertexOutput GetVertexOutput(VertexInputs input, float rand, WindSettings s, BendSettings b)
{
	...
	//Note: Camera up vector works better, but breaks in shadow pass!
	float3 pushVector = -data.viewDir;
	//Push grass away from camera position (billboard against near-clip plane)
	offsets.xz += pushVector.xz * b.mask* b.perspectiveCorrection;
