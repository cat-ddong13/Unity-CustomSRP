    ...
    public class StylizedGrassRenderer : MonoBehaviour
    {
        ...
        private Camera CreateCamera()
        {
            ...
            if (UniversalRenderPipeline.asset)
            {
                ...
                if (!bendRenderer) bendRenderer = PipelineUtilities.GetRenderer((DrawGrassBenders.AssetName));
        ...
        private static float SnapToGrid(float pos, float cellSize)
        {
            return Mathf.FloorToInt(pos / cellSize) * (cellSize) + (cellSize * 0.5f);
        }
