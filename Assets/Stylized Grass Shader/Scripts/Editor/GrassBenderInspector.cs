...
namespace StylizedGrass
{
    ...
    public class GrassBenderInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (benderType.intValue == (int)GrassBenderBase.BenderType.Line)
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    EditorGUILayout.PropertyField(lineRenderer);

                    using (new EditorGUI.DisabledGroupScope(bender.lineRenderer ? bender.lineRenderer.gameObject == bender.gameObject : false))
                    {
                        if (GUILayout.Button("This", EditorStyles.miniButton, GUILayout.MaxWidth(75f)))
                        {
                            LineRenderer lineRend = bender.GetComponent<LineRenderer>();
                            if (lineRend) lineRenderer.objectReferenceValue = lineRend;
                        }
                    }
                }
                ...
                StylizedGrassGUI.ParameterGroup.DrawHeader(new GUIContent("Line"));
                using (new EditorGUILayout.VerticalScope(StylizedGrassGUI.ParameterGroup.Section))
                {
                    EditorGUILayout.PropertyField(alphaBlending);
                }
